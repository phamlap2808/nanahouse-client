...
<template>
  <div class="category-page p-4">
    ...
    <div v-loading="loading" class="min-h-100">
      <v-data-table-server
v-model:items-per-page="totalPageRecord" :headers="headers" :items="listCategory"
        :loading="loading" loading-text="Tải dữ liệu" class="elevation-1" :show-current-page="true"
        :items-length="totalRecord" @update:options="getListCategory">
    ...
    <v-select
v-model="categoryHome" :flat="true" :items="listCategoryChild" :chips="true" :multiple="true"
      item-title="category_name" item-value="category_id" />
    <v-btn @click="HandleListCategoryHome">Cập nhật trang chủ</v-btn>

    <admin-category-dialog-create-category
v-if="showCreateModal" :is-open="showCreateModal"
      :toggle-open="toggleCreateModal" @refecth="createCategorySuccess" />
    <admin-category-dialog-edit-category
v-if="showEditModal" :is-open="showEditModal" :toggle-open="toggleEditModal"
      :item-edit="selectionItem" @refecth="createCategorySuccess" />
  </div>
</template>